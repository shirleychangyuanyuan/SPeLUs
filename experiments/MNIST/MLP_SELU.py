...
def lselu(x):
    with ops.name_scope("lselu") as scope:
        a = 1.04635347140378
        b = 1.7120011458843978
        x = ops.convert_to_tensor(x)
        ones = array_ops.ones_like(x, x.dtype)
        zeros = array_ops.zeros_like(x, x.dtype)
        mask_0 = tf.where(x >= 0.0, ones, zeros)
        mask_1 = tf.where((x >= -0.5) & (x < 0), ones, zeros)
        mask_2 = tf.where((x >= -1) & (x < -0.5), ones, zeros)
        mask_3 = tf.where((x >= -1.5) & (x < -1), ones, zeros)
        mask_4 = tf.where((x >= -2) & (x < -1.5), ones, zeros)
        mask_5 = tf.where((x >= -2.5) & (x < -2), ones, zeros)
        mask_6 = tf.where((x >= -3) & (x < -2.5), ones, zeros)
        mask_7 = tf.where((x >= -3.5) & (x < -3), ones, zeros)
        mask_8 = tf.where((x >= -4) & (x < -3.5), ones, zeros)
        mask_9 = tf.where((x >= -4.5) & (x < -4), ones, zeros)
        mask_10 = tf.where((x >= -5) & (x < -4.5), ones, zeros)
        mask_11 = tf.where((x >= -6) & (x < -5), ones, zeros)
        mask_12 = tf.where((x >= -7) & (x < -6), ones, zeros)
        mask_13 = tf.where(x < -7, ones, zeros)
        y_positive = mask_0 * x
        y_negative = mask_1 * 0.7869 * x + mask_2 * (-0.1548 + 0.4773 * x) + mask_3 * (
        -0.3426 + 0.2895 * x) + mask_4 * (-0.5135 + 0.1756 * x) + mask_5 * (-0.6517 + 0.1065 * x) + mask_6 * (
        -0.7564 + 0.0646 * x) + mask_7 * (-0.8327 + 0.03918 * x) + mask_8 * (-0.8866 + 0.02376 * x) + mask_9 * (
        -0.924 + 0.01441 * x) + mask_10 * (-0.9496 + 0.008742 * x) + mask_11 * (-0.972 + 0.004259 * x) + mask_12 * (
        -0.9881 + 0.001567 * x) + mask_13 * (-0.999)
...
def weight_variable(shape):
    initial=tf.get_variable('weight',shape=shape,initializer=tf.random_normal_initializer(stddev=np.sqrt(1/shape[0])))
...
acc_tr_op=tf.summary.scalar("train_accuracy", accuracy)
acc_te_op=tf.summary.scalar("test_accuracy",accuracy)
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options))as sess:
    sess.run(init)
    writer = tf.summary.FileWriter('MNIST_EPOCH/selu%d' % 32, sess.graph)
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            if epoch % display_step == 0:
                loss_SELU, acc_SELU, summary_tr = sess.run([cost, accuracy, smry_tr], feed_dict={x: batch_x_norm,
                                                                                                 y: batch_y,
                                                                                                 dropout_prob: 0.0,
                                                                                               is_training: False})
                ...
                msg = "epoch {0:>6}:" \
                  "accuracy SELU: {1:>6.1%} " \
                  "loss  SELU: {2:.2f} "
                print(msg.format(epoch + 1, acc_SELU, loss_SELU))

                acc_SELU_test, summary_te = sess.run([accuracy, smry_va],
                ...
                mgs = "Accuracy on Test-Set SELU:{0:>6.1%} "
                print(mgs.format(acc_SELU_test))
    ...
    #               "accuracy SELU: {1:>6.1%} " \
    #               "loss  SELU: {2:.2f} "
    #         print(msg.format(step + 1, acc_SELU, loss_SELU))
    #
    #         acc_SELU_test, summary_te = sess.run([accuracy, smry_va],
    #                                                feed_dict={x: scaler.transform(mnist.test.images[:1000]),
    #                                                           y: mnist.test.labels[:1000],
