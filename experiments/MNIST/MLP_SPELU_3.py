...
acc_tr_op = tf.summary.scalar("train_acc", accuracy)
acc_te_op = tf.summary.scalar("test_acc", accuracy)
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options))as sess:
    sess.run(init)
    writer = tf.summary.FileWriter('MNIST_EPOCH/spelu3%d' % 32, sess.graph)
