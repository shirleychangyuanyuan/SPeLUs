    ...
    def _build_graph(self, inputs):
        ...
        def selu(x,name='selu'):
            with ops.name_scope(name) as scope:
                alpha = 1.6732632423543772848170429916717
                scale = 1.0507009873554804934193349852946
                return scale * tf.where(x >= 0.0, x, alpha *(tf.exp(x)-1))

        def elu_network(x):
            # stack1
            conv1_stack1 = Conv2D('stack1/conv1', x, out_channel=32, kernel_shape=5, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (5 * 5 * 3))), nl=selu)
           # add_activation_summary(conv1_stack1, name='selu')
            ...
            conv1_stack2 = Conv2D('stack2/conv1', pool1, out_channel=64, kernel_shape=3, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*32))), nl=selu)
            #add_activation_summary(conv1_stack2, name='selu')
            conv2_stack2 = Conv2D('stack2/conv2', conv1_stack2, out_channel=64, kernel_shape=3, padding='SAME',
                                  stride=1,W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9 * 64))), nl=selu)
            #add_activation_summary(conv2_stack2, name='selu')
            ...
            conv1_stack3 = Conv2D('stack3/conv1', pool2, out_channel=128, kernel_shape=3, padding='SAME',
                                  stride=1, W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*64))), nl=selu)
            #add_activation_summary(conv1_stack3, name='selu')
            conv2_stack3 = Conv2D('stack3/conv2', conv1_stack3, out_channel=128, kernel_shape=2, padding='SAME',
                                  stride=1, W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (4 * 128))),nl=selu)

            #add_activation_summary(conv2_stack3, name='selu')
            ...
            conv1_stack4 = Conv2D('stack4/conv1', pool3, out_channel=256, kernel_shape=3, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*128))), nl=selu)

            #add_activation_summary(conv1_stack4, name='selu')
            conv2_stack4 = Conv2D('stack4/conv2', conv1_stack4, out_channel=256, kernel_shape=2, padding='SAME',
                                  stride=1,W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (4 * 256))), nl=selu)

            #add_activation_summary(conv2_stack4, name='selu')
            ...
            conv1_stack5 = Conv2D('stack5/conv1', pool4, out_channel=280, kernel_shape=3, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*256))), nl=selu)

           # add_activation_summary(conv1_stack5, name='selu')
            conv2_stack5 = Conv2D('stack5/conv2', conv1_stack5, out_channel=280, kernel_shape=2, padding='SAME',
                                  stride=1,W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (4 * 280))), nl=selu)

            #add_activation_summary(conv2_stack5, name='selu')
            ...
            conv1_stack6 = Conv2D('stack6/conv1', pool5, out_channel=300, kernel_shape=3, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*280))), nl=selu)

            #add_activation_summary(conv1_stack6, name='selu')
            ...
            conv1_stack7 = Conv2D('stack7/conv1', stack6_dropout, out_channel=100, kernel_shape=1, padding='SAME',
                                  stride=1,W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / 300)), nl=selu)

            #add_activation_summary(conv1_stack7, name='selu')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-g", "--gpuid", type=str, help='GPU to use (leave blank for CPU only)', default="2")
