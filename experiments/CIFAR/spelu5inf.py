    ...
    def _build_graph(self, inputs):
        ...
        def lselu_3(x,name='lselu3'):
            with ops.name_scope(name)as scope:
                # a = 1.0347512791111948
                # b = 1.8342374136427047
                a=1.035
                b=1.834
                ...
                mask_1 = tf.where((x >= -1) & (x < 0), ones, zeros)
                mask_2 = tf.where((x >= -2) & (x < -1), ones, zeros)
                mask_3 = tf.where((x >= -3.5) & (x < -2), ones, zeros)
                mask_4 = tf.where(x < -3.5, ones, zeros)
                y_positive = mask_0 * x
                y_negative = mask_1 * 0.6321 * x + mask_2 * (-0.3996 + 0.2325 * x) + mask_3 * (
                    -0.7245 + 0.07009 * x) + mask_4 * (0.005468 * x - 0.9507)
        ...
        def elu_network(x):
            ...
            conv1_stack1 = Conv2D('stack1/conv1', x, out_channel=32, kernel_shape=5, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (5 * 5 * 3))), nl=lselu_3)
            #add_activation_summary(conv1_stack1, name='lselu')
            ...
            conv1_stack2 = Conv2D('stack2/conv1', pool1, out_channel=64, kernel_shape=3, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*32))), nl=lselu_3)
            #add_activation_summary(conv1_stack2, name='lselu')
            conv2_stack2 = Conv2D('stack2/conv2', conv1_stack2, out_channel=64, kernel_shape=3, padding='SAME',
                                  stride=1,W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9 * 64))), nl=lselu_3)
            #add_activation_summary(conv2_stack2, name='lselu')
            ...
            conv1_stack3 = Conv2D('stack3/conv1', pool2, out_channel=128, kernel_shape=3, padding='SAME',
                                  stride=1, W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*64))), nl=lselu_3)
            #add_activation_summary(conv1_stack3, name='lselu')
            conv2_stack3 = Conv2D('stack3/conv2', conv1_stack3, out_channel=128, kernel_shape=2, padding='SAME',
                                  stride=1, W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (4 * 128))),nl=lselu_3)

            #add_activation_summary(conv2_stack3, name='lselu')
            ...
            conv1_stack4 = Conv2D('stack4/conv1', pool3, out_channel=256, kernel_shape=3, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*128))), nl=lselu_3)

            #add_activation_summary(conv1_stack4, name='lselu')
            conv2_stack4 = Conv2D('stack4/conv2', conv1_stack4, out_channel=256, kernel_shape=2, padding='SAME',
                                  stride=1,W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (4 * 256))), nl=lselu_3)

            #add_activation_summary(conv2_stack4, name='lselu')
            ...
            conv1_stack5 = Conv2D('stack5/conv1', pool4, out_channel=280, kernel_shape=3, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*256))), nl=lselu_3)

            #add_activation_summary(conv1_stack5, name='lselu')
            conv2_stack5 = Conv2D('stack5/conv2', conv1_stack5, out_channel=280, kernel_shape=2, padding='SAME',
                                  stride=1,W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (4 * 280))), nl=lselu_3)

            #add_activation_summary(conv2_stack5, name='lselu')
            ...
            conv1_stack6 = Conv2D('stack6/conv1', pool5, out_channel=300, kernel_shape=3, padding='SAME', stride=1,
                                  W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / (9*280))), nl=lselu_3)

            #add_activation_summary(conv1_stack6, name='lselu')
            ...
            conv1_stack7 = Conv2D('stack7/conv1', stack6_dropout, out_channel=100, kernel_shape=1, padding='SAME',
                                  stride=1,W_init=tf.random_normal_initializer(stddev=np.sqrt(1 / 300)), nl=lselu_3)

            #add_activation_summary(conv1_stack7, name='lselu')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-g", "--gpuid", type=str, help='GPU to use (leave blank for CPU only)', default="14")
