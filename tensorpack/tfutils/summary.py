...
from .tower import get_current_tower_context#同一个目录下
...
def add_activation_summary(x, name=None):
    ...
    if name is None:
        name = x.name
    #_mean, _variance = tf.nn.moments(x, axes=[0,2,3])#计算channel维度上的均值，方差
    _mean, _variance = tf.nn.moments(x, axes=[0, 1,2, 3])
    with tf.name_scope('activation'):
        ...
        # tf.summary.scalar(name+'-variance',variance)
        # tf.summary.scalar(name + '/channel01_mean', _mean[0])
        # tf.summary.scalar(name + '/channel01_variance', _variance[0])
...
def add_param_summary(*summary_lists):
    ...
    def perform(var, action):
        ...
        if action == 'scalar':
            assert ndim == 0, "Scalar summary on high-dimension data. Maybe you want 'mean'?"#断言成功，不执行任何操作
