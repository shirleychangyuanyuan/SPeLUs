    ...
    def _build_graph(self, inputs):
        ...
        def lselu_3(x,name='lselu3'):
            with ops.name_scope(name)as scope:
                a = 1.0140176710063815
                b =2.2324871468218817
                x = ops.convert_to_tensor(x)
                ones = array_ops.ones_like(x, x.dtype)
                zeros = array_ops.zeros_like(x, x.dtype)
                mask_0 = tf.where(x >= 0.0, ones, zeros)
                mask_1 = tf.where((x >= -1.8) & (x < 0), ones, zeros)
                mask_2 = tf.where(x < -1.8, ones, zeros)
                y_positive = mask_0 * x
                y_negative = mask_1 * 0.4637 * x + mask_2 * (0.02294 *x - 0.7934)
